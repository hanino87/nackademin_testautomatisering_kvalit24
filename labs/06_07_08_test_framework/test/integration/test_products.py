...
def test_add_product_to_catalog():
   ...
   Then The product is available to be used in the app"""
    ...
    already_exists = any(p.get("name") == "apple" for p in products_before)

    # When I add a product to the catalog​
    create_response = admin_api.create_product("apple")
    assert create_response.status_code == 200
    
    # Then The product is available to be used in the app"
    # Verify product exists
    products_after = admin_api.get_products_list()
    assert any(p.get("name") == "apple" for p in products_after)
    # Verify count
    ...
    print("✅ Products in catalog after add:", products_after)
...
def test_remove_product_from_catalog():
    ...
    # Delete product (safe to call even if not present)
