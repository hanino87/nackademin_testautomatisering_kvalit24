...
def test_add_product_to_catalog(page: Page):
    ...
    product_page.add_product("Fish")
    product_page.add_product("Bajen Shirt")
    
    # Assert the latest added "Bajen Shirt" is visible (duplicate-safe)
    expect(product_page.product_lists.get_by_text("Bajen Shirt").nth(-1)).to_be_visible()
    expect(product_page.product_lists.get_by_text("Bajen Shirt").nth(-1)).not_to_be_hidden()
    
    # Assert that the list dont contain an product i havent "added"
    expect(product_page.product_lists.get_by_text("Computer")).not_to_be_visible()
    expect(product_page.product_lists.get_by_text("Computer")).to_be_hidden()
    
    # Assert that the list contain Fish is visbile (not duplicate product)
    expect(product_page.product_lists.get_by_text("Fish")).to_be_in_viewport()
    
    # Assert list of products is not empty anymore after adding products
    expect(product_page.empty_product_message).not_to_be_in_viewport()
    
    # Teardown    
    product_page.logout_from_store()
...
def test_remove_product_from_catalog(page: Page):
    ...
    home_page.navigate()
    login_page.login_with_user_that_exist_in_database("testuser","secret")
    product_page.delete_product_by_name(("Fish"))
    
    # Assert that the Fish product is gone
    expect(product_page.product_lists.get_by_text("Fish")).to_be_hidden()
    expect(product_page.product_lists).not_to_contain_text("Fish")
    
    # Assert that the other products in the store has not been deleted 
    expect(product_page.product_lists).to_contain_text("Bajen Shirt")
    
    # Assert that by deleting just one product the store is not empty of products
    expect(product_page.empty_product_message).to_be_hidden()
    
    product_page.delete_all_products_in_store()
    
    # Assert that all products in store is gone the product catalog should be empty 
    expect(product_page.empty_product_message).to_be_visible()
