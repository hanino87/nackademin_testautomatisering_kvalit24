...
def generate_password(prefix: str = "pass", length: int = 8) -> str:
    """
    Generate a password with a configurable prefix and a random string.

    Args:
        prefix (str): The prefix for the username (default: "pass").
        length (int): Length of the random string to append (default: 8).

    Returns:
        str: A password like "pass_ab12cd34".
    """
    random_part = ''.join(random.choices(string.ascii_lowercase + string.digits, k=length))
    return f"{prefix}_{random_part}"